...
class PatientDataValidator:
    ...
    def validate_name(name: str) -> Tuple[bool, str]:
        ...
        if not re.match(allowed_pattern, name):
            ...
            return False, f"Name contains invalid characters: {', '.join(set(invalid_chars))}. Only letters, spaces, hyphens, and apostrophes allowed."
    ...
    def validate_dob(dob_str: str, allow_future: bool = False) -> Tuple[bool, str]:
        ...
        if dob_obj is None:
            return False, "Invalid date format. Please use YYYY-MM-DD (e.g., 1990-05-15). Other formats accepted: MM/DD/YYYY or DD/MM/YYYY"
...
class SchedulingValidator:
    ...
    def validate_appointment_date(date_str: str) -> Tuple[bool, str]:
        ...
        if apt_date is None:
            return False, "Invalid date format. Please use YYYY-MM-DD (e.g., 2026-04-15). Accepts MM/DD/YYYY or DD/MM/YYYY"
        ...
        if apt_date_only < today:
            ...
            return False, f"That date is {days_in_past} days in the past. Please select a future date"
    ...
    def validate_time_slot(time_str: str) -> Tuple[bool, str]:
        ...
        return False, "Invalid time format. Please use HH:MM (24-hour, e.g., 14:30) or HH:MM AM/PM (e.g., 2:30 PM)"
    ...
    def validate_doctor_name(doctor_name: str, available_doctors: List[str]) -> Tuple[bool, str]:
        ...
        return False, f"Doctor '{doctor_name}' not found. Available doctors: {', '.join(available_doctors)}"
    
    @staticmethod
    def validate_duration_match(appointment_duration: int, duration_options: List[int] = None) -> Tuple[bool, str]:
        """
        ...
        return False, f"Invalid duration {appointment_duration}. Valid durations: {', '.join(str(d) for d in duration_options)} minutes"
...
class InsuranceValidator:
    ...
    def validate_insurance_carrier(carrier: str, valid_carriers: List[str]) -> Tuple[bool, str]:
        ...
        return False, f"Carrier '{carrier}' not recognized. Valid carriers: {', '.join(valid_carriers)}"
    ...
    def validate_member_id(member_id: str) -> Tuple[bool, str]:
        ...
        if len(member_id) < 3:
            return False, f"Member ID too short ({len(member_id)} chars). Minimum 3 characters required"
    ...
    @staticmethod
    def validate_all_insurance_fields(carrier: str, member_id: str, group_id: str,
                                     valid_carriers: List[str]) -> Tuple[bool, Dict]:
        """
        ...
        # Validate each field
        carrier_valid, carrier_msg = InsuranceValidator.validate_insurance_carrier(carrier, valid_carriers)
        if carrier_valid:
...
class EdgeCaseValidator:
    ...
    @staticmethod
    def check_overlapping_bookings(new_date: str, new_time: str, new_duration: int,
        ...
        for apt in existing_appointments:
            try:
                ...
                if (new_start < existing_end) and (new_end > existing_start):
                    ...
                    return False, f"Slot conflicts with existing appointment ({existing_start.strftime('%H:%M')} - {existing_end.strftime('%H:%M')})"
            except ValueError:
                continue
