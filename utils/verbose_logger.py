...
class VerboseLogger:
    ...
    def node_end(self, node_name: str, response: str = ""):
        print(f"{Colors.GREEN}✅ NODE COMPLETE: {node_name}{Colors.RESET}"
              f"  [{self._ts()}]")
        if response:
    ...
    def tool_call(self, tool_name: str, params: dict = None):
        print(f"{Colors.YELLOW}🔧 TOOL CALLED  : {tool_name}{Colors.RESET}"
              f"  [{self._ts()}]")
        if params:
    ...
    def tool_result(self, tool_name: str, result, success: bool = True):
        ...
        print(f"{color}{icon} TOOL RESULT    : {tool_name}{Colors.RESET}"
              f"  [{self._ts()}]")
    ...
    def llm_call(self, provider: str, model: str, phase: str):
        print(f"{Colors.BLUE}🧠 LLM CALL     : {provider} / {model}{Colors.RESET}"
              f"  [{self._ts()}]")
        print(f"{Colors.BLUE}   ├─ Phase      : {phase}{Colors.RESET}")

    def llm_result(self, intent: str, extracted: dict, phase: str):
        print(f"{Colors.BLUE}🧠 LLM DECISION : intent={intent} | phase={phase}{Colors.RESET}"
              f"  [{self._ts()}]")
    ...
    def state_update(self, field: str, value: str, action: str = "SET"):
        ...
        print(f"{color}📝 STATE {action:<6} : {field} = {value}{Colors.RESET}"
              f"  [{self._ts()}]")
    ...
    def state_summary(self, state: dict):
        ...
        for key, label in fields:
            ...
            tick  = "✅" if val else "❌"
            print(f"   {tick} {Colors.CYAN}{label:<14}{Colors.RESET}"
                  f"{color}{val or 'missing'}{Colors.RESET}")

    def missing_fields(self, missing: list):
        if missing:
            print(f"{Colors.ORANGE}⚠️  MISSING FIELDS: "
                  f"{', '.join(missing)}{Colors.RESET}  [{self._ts()}]")
        else:
            print(f"{Colors.GREEN}✅ ALL FIELDS COLLECTED — ready to confirm!"
                  f"{Colors.RESET}  [{self._ts()}]")

    def booking_event(self, event: str, details: dict = None):
        print(f"{Colors.GREEN}{Colors.BOLD}🎉 BOOKING EVENT: {event}{Colors.RESET}"
              f"  [{self._ts()}]")
        if details:
    ...
    def error(self, where: str, msg: str):
        print(f"{Colors.RED}❌ ERROR in {where}: {msg}{Colors.RESET}"
              f"  [{self._ts()}]")

    def warning(self, msg: str):
        print(f"{Colors.ORANGE}⚠️  WARNING: {msg}{Colors.RESET}"
              f"  [{self._ts()}]")

    def doctor_change(self, old: str, new: str):
        print(f"{Colors.ORANGE}🔄 DOCTOR CHANGE: {old or 'None'} → {new}{Colors.RESET}"
              f"  [{self._ts()}]")

    def phase_change(self, old: str, new: str):
        print(f"{Colors.MAGENTA}🔀 PHASE CHANGE : {old} → {new}{Colors.RESET}"
              f"  [{self._ts()}]")
