...
class ReminderService:
    ...
    def setup_appointment_reminders(self, appointment_id: str,
            ...
            reminders = [
                {
                    ...
                    'scheduled_time': (
                        apt_dt - timedelta(hours=48)).isoformat()
                },
                {
                    'reminder_id':    f"{appointment_id}_R2",
                    'reminder_type':  'General Reminder',
                    'scheduled_time': (
                        apt_dt - timedelta(hours=24)).isoformat()
                },
                {
                    'reminder_id':    f"{appointment_id}_R3",
                    'reminder_type':  'Confirmation',
                    'scheduled_time': (
                        apt_dt - timedelta(hours=1)).isoformat()
                }
            ]

            from database.db import save_reminders
            save_reminders(appointment_id, email or '',
                           phone or '', reminders)

            logger.info(
                f"3 reminders saved to DB: {appointment_id}")
                ...
                'reminder_schedule': [
                    {
                        ...
                        'message':     'Please confirm if you '
                                       'have filled your forms.'
                    },
                    {
                        'position':    2,
    ...
    def send_form_reminder(self,
        ...
        try:
            ...
            # Mock send
            logger.info(
                f"Form reminder sent for {appointment_id}")
            return {'success': True,
