...
def _resolve_relative_date(text: str) -> str:
    ...
    if any(
        word in t
        for word in ["asap", "as soon as possible", "urgent", "immediately", "right away", "now"]
    ):
        return str(today)
...
def conversation_node(state: dict) -> dict:
    ...
    answering_for_someone = (
        state.get("booking_for_person")
        and not state.get("booking_for_someone_else_confirmed")
        and any(
            x in inp_lower
            for x in [
                "for my",
        ...
        and len(user_input.split()) < 10  # short answer = responding to question
    )
    ...
    detected_person = next((t for t in third_person_triggers if t in inp_lower), None)
    ...
    _input_is_exact_doctor = any(
        doc["name"].lower() == user_input.lower() or doc["name"] == user_input
    ...
    if state.get("available_slots"):
        ...
        and not state.get("selected_time")
    ):
        slots_block = (
    ...
    if (
        ...
        try:
            ...
            if avail.get("available"):
                ...
                time_match = (
                    next((s for s in slots if preferred_time in s or s in preferred_time), None)
                    if preferred_time
                    else None
                )
    ...
    if (
        confirm_intent
        and not state.get("selected_time")
        and state.get("preferred_doctor")
        and state.get("appointment_date")
    ):
...
def _update_state_from_extracted(state: dict, extracted: dict) -> None:
    if extracted.get("patient_name") and not state.get("patient_name"):
        ...
        is_junk = (
            len(raw_name.strip()) < 2
            or any(p in raw_name.lower() for p in junk_patterns)
            or not any(c.isalpha() for c in raw_name)
        )
        if not is_junk:
...
def _rule_based_fallback(state: dict, user_input: str) -> dict:
    ...
    response = (
        ...
        if missing
        else "I have all your details! Shall I confirm your appointment? (yes/no)"
    )
    return {
        "intent": "provide_info",
        "extracted": extracted,
